...
WaveletBundle = namedtuple(
    'WaveletBundle', ['EPSD_ensemble_mean', 'freqs', 't_axis', 'EPSD_ensemble_all'])
...
class CWTx():
    ...
    def goto_scales(self,):
        _goto_scales = np.logspace(
            start=2, stop=10, num=30,  base=2, endpoint=True)
        console.print(
            f'(a={2},b={10}) ==> scales_range({_goto_scales[0]}, {_goto_scales[-1]}) ==> {self.freqhelper(_goto_scales)}')
        return _goto_scales
    ...
    def check_scales(self, a, b, num):
        ...
        checked_scales = np.logspace(
            start=a, stop=b, num=num,  base=2, endpoint=True)
        console.print(
            f'(a={a},b={b}) ==> scales_range({checked_scales[0]}, {checked_scales[-1]}) ==> {self.freqhelper(checked_scales)}')

    def propose_scales(self, a, b, num):
        """ propose scales for later computation """

        self._proposed_scales = np.logspace(
            start=a, stop=b, num=num,  base=2, endpoint=True)
    ...
    def computeEPSD(self, externaldata=None):
        ...
        if externaldata is not None:
            console.print(
                "Yo! Computing EPSD from external data with the proposed scales")
    ...
    def g_a_SRMsimu(self, external_EPSD=None):
        ...
        if external_EPSD is None:
            ...
            trial_simulation = SRM_formula(
                external_EPSD[0], external_EPSD[1], external_EPSD[2])
            return trial_simulation
    ...
    def g_ensemble_simus(self, ensemble_size, external_EPSD=None):
        ...
        if external_EPSD is None:
            ...
            single_realization_func = partial(
                self.g_a_SRMsimu, external_EPSD=external_EPSD)
            ensemble_list = [single_realization_func()
    ...
    def EPSD_UncertaintyAlongTime(self, external_EPSDbundle, fixedFreqIndex=250, time_range=[6, 7, 8]):
        ...
        for time in time_range:
            Sft_by_time[f't{time}'] = external_EPSDbundle.EPSD_ensemble_all[fixedFreqIndex, :, int(
                time * self.fs)]
            ground_truth.append(
                self._pwr_coef[fixedFreqIndex, int(time * self.fs)])
        ...
        for gt in ground_truth:
            ax.axvline(x=gt, ymin=0, ymax=1, linestyle=next(
                line_styles_cycles), linewidth=1, color='purple')
    ...
    def EPSD_uncertainty_givenranges(self, external_EPSDbundle, style='fillin'):
        ...
        for i in CriterionArray:
            ...
            cordinates_one = np.argwhere((upper_lmt > external_EPSDbundle[0]) & (
                external_EPSDbundle[0] > lower_lmt))
        ...
        for item in cordinates_ofinterest:
            x, y = item
            _a_containter.append(
                external_EPSDbundle.EPSD_ensemble_all[x, :, y])
        ...
        df = pd.DataFrame(
            {'Swt': for_the_data, 'Location': for_the_identifier})
        ...
        for row in cordinates_ofinterest:
            x, y = row
            the_labels.append(
                r"$f=$" + f"{external_EPSDbundle[1][x]:.1f}" + ', ' + r"$t=$" + f"{external_EPSDbundle[2][y]:.1f}")
        ...
        for gt, color in zip(ground_truth, colors):
            ax.axvline(x=gt, ymin=0, ymax=1, color=color,
